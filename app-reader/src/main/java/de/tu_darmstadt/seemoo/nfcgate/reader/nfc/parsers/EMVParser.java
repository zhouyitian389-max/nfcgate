    ...
    public static EMVData parse(byte[] data) {
        ...
        try {
            ...
            if (elapsedMs > SLOW_PARSE_THRESHOLD_MS) {
                System.err.println(TAG + ": Slow parse: " + elapsedMs + "ms for " + (data != null ? data.length : 0) + " bytes");
            }
            return result;
        } catch (RuntimeException e) {
            System.err.println(TAG + ": Parse failed for data length: " + (data != null ? data.length : 0));
            return null;
        }
    }
